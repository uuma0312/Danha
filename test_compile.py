...
from danha_compile import compile_module, run_native
from danha_parser import parse
from danha import _find_engine_symbol_leaks
...
def compile_ir(source: str) -> str:
    ast = parse(lex(source), source_code=source)
    module = compile_module(ast, source_code=source)
    return str(module)
...
""",
    ...
    # HashMap remove keeps collision chain reachable ("a" and "q" collide in initial table)
    """
m = HashMap.new()
m.set("a", 1)
m.set("q", 17)
...
m.set("alive", true)
print(m.get_i32("hp"))
print(m.get_f64("speed"))
print(m.get_str("name"))
print(m.get_bool("alive"))
""",

    # HashMap generic type annotation surface
    """
...
""",
    ...
    # HashMap explicit capacity
    """
m: HashMap<str, i32> = HashMap.new(2048)
i = 0
while i < 1100 {
    m.set(to_string(i), i)
    i = i + 1
...
print(m.len())
print(m.get_i32("1099"))
""",

    # HashMap non-power-of-two capacity
    """
m: HashMap<str, i32> = HashMap.new(1000)
i = 0
while i < 900 {
    m.set(to_string(i), i)
    i = i + 1
...
print(m.len())
print(m.get_i32("0"))
print(m.get_i32("1024"))
print(m.get_i32("1299"))
""",
...
""",
    ...
    # 함수 포인터: struct field 콜백 호출
    """
struct CallbackSlot {
    cb: fn(i32) -> i32
}
fn add_ten(x: i32) -> i32 { return x + 10 }
slot = CallbackSlot { cb: add_ten }
print(slot.cb(5))
""",

    # 함수 포인터: struct field 교체 후 호출
    """
struct CallbackSlot {
    cb: fn(i32) -> i32
}
fn add_one(x: i32) -> i32 { return x + 1 }
fn add_two(x: i32) -> i32 { return x + 2 }
slot = CallbackSlot { cb: add_one }
print(slot.cb(10))
slot.cb = add_two
print(slot.cb(10))
""",

    # 함수 포인터: 배열 field 안의 콜백 호출
    """
struct CallbackTable {
    callbacks: [fn(i32) -> i32; 2]
}
fn left(x: i32) -> i32 { return x - 1 }
fn right(x: i32) -> i32 { return x + 1 }
table = CallbackTable { callbacks: [left, right] }
print(table.callbacks[0](20))
print(table.callbacks[1](20))
""",
...
def normalize_floats(text):
    ...
    import re
    def _normalize_numeric_text(line):
        line = re.sub(r'(?<![.\d])(-?\d+)\.0(?!\d)', r'\1', line)
        return re.sub(r'(?<![\w.])-0(?![\w.])', '0', line)
    ...
    for line in text.split('\n'):
        ...
        if re.match(r'^vec[234]\(', line):
            ...
            line = _normalize_numeric_text(line)
            out.append(line)
        ...
        # EnumName.Variant(...) 안의 숫자.0을 정규화
            ...
            out.append(_normalize_numeric_text(line[:-2]))
        else:
            out.append(_normalize_numeric_text(line))
    return '\n'.join(out)
...
print(abs(-3))
    ...
    # ===== Danha core standalone guard =====
    # 일반 언어 기능은 Ari/SDL/GL 같은 엔진 심볼을 암묵적으로 끌고 오면 안 된다.
    # 필요한 엔진 결합은 명시적 import/@clink/호출 뒤에만 생겨야 한다.
    CORE_FORBIDDEN_SYMBOLS = [
        'ari_save_open', 'ari_save_int', 'ari_save_float', 'ari_save_string',
        'ari_save_close', 'ari_load_open', 'ari_load_int', 'ari_load_float',
        'ari_load_close',
    ]
    CORE_STANDALONE_CASES = [
        ("일반 struct", """
struct Player { hp: i32, atk: i32 }
p = Player { hp: 100, atk: 25 }
print(p.hp)
        """),
        ("reflection dump", """
struct Stats { hp: i32, speed: f64 }
s = Stats { hp: 10, speed: 2.5 }
_reflect_Stats_dump(&s)
        """),
        ("attribute struct", """
@serialize
struct SaveData { score: i32 }
d = SaveData { score: 42 }
print(d.score)
        """),
    ]
    explicit_persistence_src = """
struct Stats { hp: i32, speed: f64 }
s = Stats { hp: 10, speed: 2.5 }
_reflect_Stats_save_to_path(&s, "stats.dat")
    """
    harmless_engine_text_src = """
print("ari_ is just text")
    """
    explicit_engine_import_src = """
import ari_sdl
print(1)
    """
    ...
    core_failed = 0
    print()
    print("=== Danha core standalone guard ===")
    for label, src in CORE_STANDALONE_CASES:
        try:
            ir_text = compile_ir(src)
            leaked = [sym for sym in CORE_FORBIDDEN_SYMBOLS if sym in ir_text]
            if leaked:
                core_failed += 1
                print(f"FAIL {label} — Ari 심볼 누수: {', '.join(leaked)}")
            else:
                core_passed += 1
                print(f"OK   {label}")
        except Exception as e:
    ...
    try:
        ir_text = compile_ir(explicit_persistence_src)
        required = ['ari_save_open', 'ari_save_int', 'ari_save_float', 'ari_save_close']
        missing = [sym for sym in required if sym not in ir_text]
        if missing:
            core_failed += 1
            print(f"FAIL explicit reflection persistence — 필요한 Ari 심볼 없음: {', '.join(missing)}")
        else:
            core_passed += 1
            print("OK   explicit reflection persistence")
    except Exception as e:
        core_failed += 1
        print(f"FAIL explicit reflection persistence — 예외: {e}")

    try:
        ir_text = compile_ir(harmless_engine_text_src)
        leaked = _find_engine_symbol_leaks(ir_text)
        if leaked:
            core_failed += 1
            print(f"FAIL harmless engine-like string — 오탐: {', '.join(leaked)}")
        else:
            core_passed += 1
            print("OK   harmless engine-like string")
    except Exception as e:
        core_failed += 1
        print(f"FAIL harmless engine-like string — 예외: {e}")

    try:
        ir_text = compile_ir(explicit_engine_import_src)
        leaked = _find_engine_symbol_leaks(ir_text)
        if leaked:
            core_passed += 1
            print("OK   explicit engine import is detected")
        else:
            core_failed += 1
            print("FAIL explicit engine import is detected — 엔진 심볼을 감지하지 못함")
    except Exception as e:
        core_failed += 1
        print(f"FAIL explicit engine import is detected — 예외: {e}")

    print()
    print(f"core standalone: {core_passed} 통과, {core_failed} 실패 / 총 {len(CORE_STANDALONE_CASES) + 3}")

    total_failed = total_failed + core_failed
